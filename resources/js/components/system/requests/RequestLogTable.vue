...
export default {
  ...
  methods: {
    ...
    async updateRequestStatus() {
      ...
      const approvedRequest = this.fetchRequests.filter(item => item.status === 'Approved');

      const data = approvedRequest.map((index,item) => {
        var dateNow = new Date()
        var currentDate = dateNow.getFullYear()+'-'+(dateNow.getMonth()+1)+'-'+dateNow.getDate()
        const date = new Date()
        const retention_date = date.getFullYear()+'-'+(date.getMonth()+1)+'-'+date.getDate()
